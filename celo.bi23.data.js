...
function account_balance (address) {
    var balances = [['usd',''],['gold',''],['lockedGold',''],['total','']];
...
//
function group_info(group_address){
...
function validatorgroup_info () {
  ...
  try {
      ...
      console.log(util.format('election_validators:%d',election_validators.length));
      //
      ...
      lines.forEach( function  (line) {
        if(line.indexOf('0x')==0 && line.indexOf('C-Labs')==-1 && line.indexOf(' false')==-1){
...
function update_groups(){
  ...
  if(groups != null){
      ...
      jsonString += 'var groups ='+JSON.stringify(groups.sort(group_descend))+';';
      //print json
...
function get_leaderboard(){
    ...
    */
    ...
    var cellRange='A3%3AK152';
    var apiKey='AIzaSyC2_r5XmQkIqOu-n0IQpXnAHBklQblGu2Q';
    var url = 'https://sheets.googleapis.com/v4/spreadsheets/'+spreadsheetId+'/values/'+cellRange+'?';
    url += 'dateTimeRenderOption=SERIAL_NUMBER&majorDimension=ROWS&valueRenderOption=UNFORMATTED_VALUE&key='+apiKey;
    var cmd = "curl \
      \'"+url+"\' \
    ...
    jsonString += 'var leader_validators ='+JSON.stringify(validators.sort(leaderboard_descend))+';';
    //print json
...
function get_networkparameters(){
  var timespans = ["updateFrequency","dequeueFrequency","queueExpiry","Approval","Referendum","Execution","unlockingPeriod","reportExpirySeconds","updatePeriod","duration"];
...
//nohup node celo.bi23.data.js transfer 3600 > "logs/transfer.`date +%F_%T`" 2>&1 &
//nohup node celo.bi23.data.js update_groups -1 > "logs/update_groups.`date +%F_%T`" 2>&1 &
//nohup node celo.bi23.data.js get_leaderboard 600 > "logs/get_leaderboard.`date +%F_%T`" 2>&1 &
//node celo.bi23.data.js get_networkparameters 0
//node celo.bi23.data.js update_all_validator_status 300 > "logs/update_all_validator_status.`date +%F_%T`" 2>&1 &
if(process.argv.length!=4)
{
  ...
  if(type == 'transfer'){
      ...
      var transfer_dollars_interval = setInterval(transfer_dollars, seconds,validator_address,validator_group_address);
  ...
  if(type == 'update_all_validator_status'){
    ...
    var update_all_validator_status_interval = setInterval(update_all_validator_status, seconds);
  }
}
